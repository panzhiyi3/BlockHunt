...
public enum ConfigC {
	...
	normal_ingameHiderDied ("%TAG%NHider %A%playername%%N killed by %A%killername%%N! %A%left%%N hider(s) remain...",
			W.messages),
	normal_ingameSeekerDied ("%TAG%NSeeker %A%playername%%N killed by %A%killername%%N! He will respawn in %A%secs%%N seconds!",
			W.messages),
	...
	normal_shopBoughtItem ("%TAG%NYou've bought the '%A%itemname%%N' item!",
			W.messages),
	normal_shopChoosenBlock ("%TAG%NYou've choosen to be a(n) '%A%block%%N' block/mob!",
			W.messages),
